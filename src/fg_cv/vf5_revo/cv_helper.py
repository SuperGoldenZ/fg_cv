...
class CvHelper:
    ...
    @staticmethod
    def all_but_red(roi, red_threshold=150):
        # Create a mask for red pixels (R >= red_threshold)
        red_mask = roi[:, :, 2] >= red_threshold

        # Invert the mask to keep all but red areas
        non_red_mask = ~red_mask

        # Create an output image initialized with zeros (black)
        output = np.zeros_like(roi)

        # Apply the non-red mask
        output[non_red_mask] = roi[non_red_mask]

        return output
    
    @staticmethod
    def keep_specific_red_to_black(roi):
        # Convert BGR to HSV for better color filtering
        hsv = cv2.cvtColor(roi, cv2.COLOR_BGR2HSV)

        # Define HSV ranges for the red shades with tolerance
        # Red can wrap around HSV hue values (0-10 and 160-180)
        
        # First red shade (#98060a) - lighter red
        lower_red1 = np.array([0, 100, 50])   # Lower bound
        upper_red1 = np.array([10, 125, 75]) # Upper bound

        # Second red shade (#751b23) - darker red
        lower_red2 = np.array([160, 100, 50])  # Lower bound
        upper_red2 = np.array([180, 125, 75]) # Upper bound
        ...
        white_background = np.full_like(roi, 255)

        # Apply mask: keep red areas and turn everything else white
        red_only = cv2.bitwise_and(roi, roi, mask=red_mask)
        red_on_white = np.where(red_mask[:, :, np.newaxis] == 0, white_background, red_only)

        # Convert red areas to black
        # Any pixel not white will be set to black
        gray = cv2.cvtColor(red_on_white, cv2.COLOR_BGR2GRAY)
        _, binary = cv2.threshold(gray, 1, 255, cv2.THRESH_BINARY_INV)

        return binary

    @staticmethod
    def all_but_red_two(roi):
        # Define lower and upper bounds in BGR
        lower_red = np.array([10, 6, 117])   # Lighter red
        upper_red = np.array([150, 150, 255])  # Darker red
